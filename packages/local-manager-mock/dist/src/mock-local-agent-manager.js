import { buildRiskEvent } from "@notch-ai-monitor/risk-policy";
import { DEFAULT_EVENT_PRIORITY, allEventsFixture, buildManagerSnapshot, confirmEventFixture, enqueueEvent, errorEventFixture, fixtureSessions, ignoreEvent, nextEventId, replaceEvents, resolveEvent, resultEventFixture, riskEventFixture, } from "@notch-ai-monitor/shared";
...
function cloneActionResult(result) {
    return {
        ...result,
...
function cloneProcessOwnership(record) {
    return {
        ...record,
        capabilities: [...record.capabilities],
    };
}
function cloneRetryLaunchProfile(record) {
    return {
        ...record,
...
function cloneProcessAudit(record) {
    return {
        ...record,
...
function cloneRetryRiskReplay(record) {
    return { ...record };
}
function cloneEventHistoryRecord(record) {
    return {
        ...record,
...
function clonePendingAction(record) {
    return { ...record };
}
function cloneProcessPersistenceSnapshot(snapshot) {
    return {
...
function cloneEventHistoryPersistenceSnapshot(snapshot) {
    return {
...
function visibilityStatusFor(resultStatus) {
    switch (resultStatus) {
...
function isTerminalActionVisibility(status) {
    return ["completed", "failed", "rejected", "noop", "expired"].includes(status);
}
function paginationOffset(cursor) {
    if (!cursor)
        return 0;
    const parsed = Number.parseInt(cursor, 10);
    return Number.isFinite(parsed) && parsed > 0 ? parsed : 0;
}
function paginationLimit(limit, fallback = 50) {
    if (limit === undefined)
        return fallback;
...
function isoPlusMilliseconds(value, milliseconds) {
    const parsed = Date.parse(value);
    if (Number.isNaN(parsed))
        return undefined;
    return new Date(parsed + milliseconds).toISOString();
}
function normalizePendingActionTimeoutMs(value) {
    if (value === null)
        return null;
    if (value === undefined)
        return DEFAULT_PENDING_ACTION_TIMEOUT_MS;
    if (!Number.isFinite(value) || value <= 0)
        return null;
    return Math.trunc(value);
}
function eventHistoryTimestamp(record) {
    return timestampValue(record.updatedAt ?? record.resolvedAt ?? record.createdAt);
}
function pendingActionTimestamp(record) {
    return timestampValue(record.updatedAt ?? record.createdAt);
}
function actionDefaultsFor(eventType, actionId) {
    const eventScoped = DEFAULT_ACTIONS_BY_EVENT_TYPE[eventType].find((action) => action.id === actionId);
    if (eventScoped)
        return eventScoped;
...
function actionForProjection(event, actionId) {
    return event.actions.find((action) => action.id === actionId) ?? actionDefaultsFor(event.type, actionId);
}
function normalizeAction(actionInput, eventType, riskMissingRiskLevel) {
    const defaults = actionDefaultsFor(eventType, actionInput.id);
    const requiresConfirm = actionInput.requiresConfirm ?? defaults.requiresConfirm ?? CONFIRMATION_ACTION_IDS.has(actionInput.id);
...
function completedMessage(action) {
    switch (action.id) {
        case "approve":
            return "已模拟批准运行";
        case "allow-once":
...
function isSafeLogTarget(value) {
...
function safeLogTargetFor(event) {
    for (const candidate of event.evidence?.affectedPaths ?? []) {
        if (isSafeLogTarget(candidate))
            return candidate.trim();
    }
    return null;
}
function mockedEffects(action, event) {
    const effect = {
...
function viewLogResult(payload, event, action, nextEventIdValue) {
...
const DEFAULT_PROCESS_CAPABILITIES = [
    "process.retry",
    "process.terminate",
];
function normalizeRetryLaunchProfile(input) {
    return {
        ...input,
        owner: "notch-manager",
...
function normalizeProcessOwnership(input) {
    return {
        ...input,
        sourceMode: "live",
        owner: "notch-manager",
...
function inputString(payload, key) {
    const value = payload.input?.[key];
    return typeof value === "string" ? value : undefined;
}
function processAuditSource(payload) {
    if (inputString(payload, "source") === "smoke")
        return "smoke";
    return payload.uiContext ? "desktop-ui" : "api";
}
function processAuditSnapshot(ownership) {
    return {
...
function retryLaunchProfileAuditSnapshot(profile) {
    return {
...
function rejectedProcessResult(payload, event, action, nextEventIdValue, code, message) {
    return {
        requestId: payload.requestId,
        eventId: payload.eventId,
        actionId: payload.actionId,
...
function processOwnershipMismatch(payload, ownership) {
...
function retryLaunchProfileMismatch(payload, profile) {
    const expectedSessionId = inputString(payload, "expectedSessionId");
...
function defaultProcessActionCompletionResolution(actionId) {
...
export class MockLocalAgentManager {
    ...
    processPersistence;
    eventHistoryPersistence;
    processSupervisor;
    pendingActionTimeoutMs;
    listeners = new Set();
    processOwnership = new Map();
    retryLaunchProfiles = new Map();
    processActionResults = new Map();
    processActionAudit = [];
    retryRiskReplays = [];
    eventHistory = [];
    eventTimeline = [];
    ...
    registerProcessOwnership(input) {
    ...
    getRetryLaunchProfile(sessionId) {
    ...
    heartbeatPendingAction(input) {
        const index = this.pendingActions.findIndex((record) => record.requestId === input.requestId && record.status === "in_progress");
        if (index < 0)
            return null;
    ...
    expirePendingActions(input = {}) {
        ...
        for (let index = 0; index < this.pendingActions.length; index += 1) {
            ...
            if (record.status !== "in_progress" || !record.expiresAt)
                continue;
            ...
            if (record.actionId === "retry" || record.actionId === "terminate") {
                const acceptedAudit = this.processActionAudit.find((item) => item.requestId === record.requestId && item.decision === "accepted");
                const hasTerminalAudit = this.processActionAudit.some((item) => item.requestId === record.requestId &&
    ...
    completeAcceptedProcessAction(input) {
        ...
        const existingPending = this.pendingActions.find((record) => record.requestId === requestId &&
            ...
            effects: [
                {
                    type: "process",
                    target: input.target ??
                        `session:${input.sessionId}#${input.status === "completed" ? resolution : `${input.actionId}-failed`}`,
                    mocked: false,
                },
            ],
            ...(input.status === "failed"
    ...
    endSession(sessionId, state, metadata = {}) {
        ...
        if (metadata.exitCode !== undefined)
            nextSession.exitCode = metadata.exitCode;
        if (metadata.endReason)
            nextSession.endReason = metadata.endReason;
    ...
    injectScenario(scenario) {
        ...
        const scenarioEvents = scenario === "idle"
            ? []
            : scenario === "waiting"
                ? [confirmEventFixture]
                : scenario === "result"
    ...
    hydrateEventHistoryPersistence() {
    ...
    rebuildEventHistoryFromCurrentEvents(source, message) {
        this.eventHistory.length = 0;
        this.eventTimeline.length = 0;
        this.pendingActions.length = 0;
        this.eventHistoryCursorVersion = 0;
    ...
    updateHistoryLatestAction(event, result, at) {
    ...
    recordPendingActionResult(input) {
        const at = input.at ?? this.clock();
        ...
        const existingIndex = this.pendingActions.findIndex((record) => record.requestId === input.result.requestId);
        ...
        const expiresAt = status === "in_progress" && this.pendingActionTimeoutMs !== null
    ...
    removeSupersededWaitingConfirmation(input) {
    ...
    rememberProcessActionResult(result) {
    ...
    finalizeActionResult(payload, event, action, result, options = {}) {
        this.recordPendingActionResult({
            result,
            event: this.events.find((item) => item.id === event.id) ?? event,
            action,
    ...
    recordProcessActionAudit(input) {
        this.processActionAudit.push({
    ...
    recordRetryRiskReplay(record) {
        this.retryRiskReplays.push(cloneRetryRiskReplay(record));
        this.persistProcessState();
    }
    confirmationRequiredProcessResult(payload, event, action) {
        const result = {
            requestId: payload.requestId,
            eventId: payload.eventId,
            actionId: payload.actionId,
    ...
    validateRetryLaunchProfile(payload, event, action, session, profile) {
        const nextEventIdValue = nextEventId(this.events);
        if (event.type !== "error") {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "retry_event_type_invalid", "只有错误事件可以重试");
        }
        if (!session) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "session_not_found", "会话不存在，无法重试");
        }
        if (session.sourceMode !== "live") {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "retry_source_not_live", "只有 Manager 托管的 live 会话可以重试");
        }
        if (!profile) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "retry_launch_profile_not_found", "没有可审计的重试启动配置");
        }
        if (profile.owner !== "notch-manager" || !profile.active) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "retry_launch_profile_inactive", "重试启动配置不可用");
        }
        if (profile.sessionId !== event.sessionId || profile.sessionId !== session.id) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "retry_launch_profile_mismatch", "重试启动配置与事件会话不匹配");
        }
        if (session.cwd && session.cwd !== profile.cwd) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "retry_launch_profile_mismatch", "重试启动配置与会话目录不匹配");
        }
        if (event.commandHash && event.commandHash !== profile.commandHash) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "retry_launch_profile_mismatch", "重试启动配置与错误事件命令不匹配");
        }
        if (!profile.capabilities.includes("process.retry")) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "process_capability_unavailable", "当前启动配置不允许重试");
        }
        if (profile.retryAttemptActive) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "retry_attempt_already_active", "已有重试尝试正在运行");
        }
        const inputMismatch = retryLaunchProfileMismatch(payload, profile);
        if (inputMismatch) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, inputMismatch.code, inputMismatch.message);
        }
        if (!this.processSupervisor?.startRetry) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "retry_supervisor_unavailable", "没有可用的受控 retry supervisor");
        }
        return null;
    }
    replayRetryRiskPolicy(payload, event, action, profile) {
    ...
    requestSupervisedRetry(payload, event, action) {
    ...
    validateTerminateOwnership(payload, event, action, session, ownership) {
        const nextEventIdValue = nextEventId(this.events);
        if (!session) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "session_not_found", "会话不存在，无法终止");
        }
        if (!ownership) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "process_ownership_not_found", "没有可审计的进程归属记录，无法终止");
        }
        if (session.sourceMode !== "live" || ownership.sourceMode !== "live") {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "process_source_not_live", "只有 Manager 托管的 live 会话可以执行终止");
        }
        if (ownership.owner !== "notch-manager" || !ownership.active) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "process_ownership_inactive", "进程归属记录不可用，无法终止");
        }
        if (ownership.sessionId !== event.sessionId || ownership.sessionId !== session.id) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "process_ownership_mismatch", "进程归属记录与事件会话不匹配");
        }
        if (session.cwd && session.cwd !== ownership.cwd) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "process_ownership_mismatch", "进程归属记录与会话目录不匹配");
        }
        if (session.processId !== undefined &&
            ownership.pid !== undefined &&
            session.processId !== ownership.pid) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "process_ownership_mismatch", "进程归属记录与会话进程不匹配");
        }
        if (!ownership.capabilities.includes("process.terminate")) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "process_capability_unavailable", "当前进程归属不允许执行终止");
        }
        ...
        if (!this.processSupervisor) {
            return rejectedProcessResult(payload, event, action, nextEventIdValue, "process_supervisor_unavailable", "没有可用的受控进程 supervisor");
        }
        return null;
    }
    requestSupervisedTerminate(payload, event, action) {
        const session = this.sessions.get(event.sessionId);
    ...
    requestAction(payload) {
        ...
        if (replayedProcessResult)
            return cloneActionResult(replayedProcessResult);
        ...
        if (this.processSideEffectMode === "supervised" &&
            action.id === "terminate" &&
        ...
        if (action.id === "view-log") {
            return this.finalizeActionResult(payload, event, action, viewLogResult(payload, event, action, nextEventId(this.events)));
        }
        ...
        const at = this.clock();
        const resolvedEventStatus = IGNORED_ACTION_IDS.has(action.id) ? "ignored" : "resolved";
        const update = resolvedEventStatus === "ignored"
    ...
    normalizeEvent(event) {
        ...
        if (event.type === "confirm" && event.command) {
            ...
            if (event.commandHash)
                riskInput.commandHash = event.commandHash;
            if (sessionCwd)
                riskInput.cwd = sessionCwd;
        ...
        const actions = (event.actions && event.actions.length > 0
            ? event.actions
            : defaultActionsFor(event.type)).map((action) => normalizeAction(action, event.type, riskMissingRiskLevel));
        ...
        if (commandHash)
            normalized.commandHash = commandHash;
        if (event.evidence) {
            normalized.evidence = {
                ...event.evidence,
        ...
        if (event.reasons)
            normalized.reasons = [...event.reasons];
        if (event.resolvedAt)
            normalized.resolvedAt = event.resolvedAt;
        if (event.resolution)
            normalized.resolution = event.resolution;
        if (event.occurrenceCount !== undefined)
            normalized.occurrenceCount = event.occurrenceCount;
        if (event.errorKey)
            normalized.errorKey = event.errorKey;
        return normalized;
    }
    emitSnapshot() {
...
export function createMockLocalAgentManager(options) {
