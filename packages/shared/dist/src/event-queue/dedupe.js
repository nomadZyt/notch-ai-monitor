...
function mergeIntoExisting(existing, incoming) {
    ...
    if (commandHash !== undefined)
        merged.commandHash = commandHash;
    ...
    if (errorKey !== undefined)
        merged.errorKey = errorKey;
    return merged;
}
function expireMergedDuplicate(event, updatedAt) {
    return {
        ...event,
        status: "expired",
        updatedAt,
...
export function applyP0Dedupe(events) {
    const next = events.map((event) => ({ ...event }));
    const resultGroups = new Map();
    next.forEach((event, index) => {
        if (event.status !== "active" || event.type !== "result")
            return;
    ...
    resultGroups.forEach((indexes) => {
        if (indexes.length <= 1)
            return;
        const keeperIndex = indexes.reduce((keeper, current) => isNewer(next[current], next[keeper]) ? current : keeper);
    ...
    const confirmByCommand = new Map();
    next.forEach((event, index) => {
        if (event.status !== "active" || event.type !== "confirm" || !event.commandHash)
            return;
    ...
    next.forEach((event, index) => {
        if (event.status !== "active" || event.type !== "error" || !event.errorKey)
            return;
