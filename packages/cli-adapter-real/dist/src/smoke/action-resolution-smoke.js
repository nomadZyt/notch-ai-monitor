...
export function parseActionResolutionSmokeArgs(argv) {
...
function snapshotFromResponse(body, context) {
...
function actionResultFromResponse(body) {
...
function findAction(actionId, actions, options) {
...
function validatePreparedSnapshot(snapshot, options) {
...
function validateActionSnapshot(snapshot, options, eventId) {
    ...
    if (snapshot.counts.activeEvents !== options.expectedActiveEvents) {
        throw new Error(`Smoke failed: active event count mismatch. Expected ${options.expectedActiveEvents}, got ${snapshot.counts.activeEvents}.`);
    }
    ...
    if (options.expectedActiveEvents > 0 &&
...
export async function runActionResolutionSmoke(argv = process.argv.slice(2)) {
    ...
    if (actionResult.status !== options.expectedResultStatus) {
        throw new Error(`Smoke failed: action result status mismatch. Expected ${options.expectedResultStatus}, got ${actionResult.status}.`);
    }
    if (options.expectedResolvedEventStatus === null && actionResult.resolvedEventStatus !== undefined) {
        throw new Error(`Smoke failed: action resolvedEventStatus should be absent, got ${actionResult.resolvedEventStatus}.`);
    }
    if (options.expectedResolvedEventStatus !== null &&
        actionResult.resolvedEventStatus !== options.expectedResolvedEventStatus) {
        throw new Error(`Smoke failed: action resolvedEventStatus mismatch. Expected ${options.expectedResolvedEventStatus}, got ${actionResult.resolvedEventStatus}.`);
    }
