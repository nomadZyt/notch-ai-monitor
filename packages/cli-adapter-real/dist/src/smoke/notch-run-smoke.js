...
import { DEFAULT_LOCAL_MANAGER_URL, runNotchRun, } from "../index.js";
const SMOKE_PROFILES = new Set([
    "codex-cli",
    "claude-code-cli",
    "qwen-cli",
    "cursor-app",
...
const SMOKE_EXPECTED_EVENT_TYPES = new Set([
    "confirm",
    "result",
    "error",
    null,
]);
function defaultChildCommand() {
    return [
        process.execPath,
...
function validateLiveSnapshot(snapshot, options) {
    ...
    if (session.exitCode !== options.expectedExitCode) {
        throw new Error(`Smoke failed: live session exitCode mismatch. Expected ${options.expectedExitCode}, got ${session.exitCode}.`);
    }
    ...
    if (options.expectedEndReason !== null && session.endReason !== options.expectedEndReason) {
        throw new Error(`Smoke failed: live session endReason mismatch. Expected ${options.expectedEndReason}, got ${session.endReason}.`);
    }
...
export async function runNotchRunSmoke(argv = process.argv.slice(2)) {
