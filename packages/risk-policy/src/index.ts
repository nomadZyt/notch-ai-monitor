...
function maxRiskLevel(levels: readonly RiskLevel[]): RiskLevel {
  return levels.reduce<RiskLevel>(
    (current, next) => (RISK_ORDER[next] > RISK_ORDER[current] ? next : current),
    "low"
  );
}
...
function isHomeDocumentsPath(path: string): boolean {
  const normalized = trimTrailingSlashes(path);
  return (
...
function isBroadRmTarget(path: string): boolean {
  const normalized = trimTrailingSlashes(path);
  return (
    normalized === "/" ||
    normalized === "/*" ||
...
function classifyRmTargets(targets: readonly string[]): RiskLevel {
  if (targets.length === 0) return "high";
  ...
  if (targets.every(isTempPath)) return "medium";

  return "high";
}

function consumeCommandArguments(tokens: readonly string[], startIndex: number): string[] {
  const args: string[] = [];
  for (let index = startIndex; index < tokens.length; index += 1) {
    const token = tokens[index];
    if (isBoundaryToken(token)) break;
    args.push(token);
  }
  return args;
}

function scanRmRecursiveForce(tokens: readonly string[]): RuleMatch[] {
...
function scanGitClean(tokens: readonly string[], context: CommandRiskContext): RuleMatch[] {
  const matches: RuleMatch[] = [];
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    const shortFlags = cleanArgs
      ...
      .join("");
    const hasDryRun =
      cleanArgs.includes("-n") || cleanArgs.includes("--dry-run") || shortFlags.includes("n");
    ...
    if (hasDryRun || !hasForce || !hasDirectories) continue;
...
function scanSudo(tokens: readonly string[]): RuleMatch[] {
  if (!tokens.some((token) => commandName(token) === "sudo")) return [];
...
function scanRecursiveWorldWritable(tokens: readonly string[]): RuleMatch[] {
  const matches: RuleMatch[] = [];

  for (let index = 0; index < tokens.length; index += 1) {
    if (commandName(tokens[index]) !== "chmod") continue;

    const args = consumeCommandArguments(tokens, index + 1);
    const hasRecursive = args.some((arg) => /^-[A-Za-z]*R[A-Za-z]*$/.test(arg));
    const modeIndex = args.findIndex((arg) => arg === "777" || arg === "0777" || arg === "a+rwx");
    if (!hasRecursive || modeIndex === -1) continue;
...
function scanCurlPipeShell(command: string): RuleMatch[] {
...
function scanDdOutput(tokens: readonly string[]): RuleMatch[] {
  const matches: RuleMatch[] = [];

  for (let index = 0; index < tokens.length; index += 1) {
    if (commandName(tokens[index]) !== "dd") continue;

    const args = consumeCommandArguments(tokens, index + 1);
    const outputArg = args.find((arg) => arg.startsWith("of="));
    if (!outputArg) continue;
...
function buildImpact(matches: readonly RuleMatch[]): string {
  if (matches.length === 0) return "未发现 P0 风险规则命中。";
  return uniqueStrings(matches.map((match) => match.impact)).join("；");
}

function buildRollback(matches: readonly RuleMatch[], riskLevel: RiskLevel): string {
  if (matches.length === 0) return "无需回滚。";
...
export function scanCommand(command: string, context: CommandRiskContext = {}): RiskAssessment {
...
export function commandLooksRisky(command: string, context: CommandRiskContext = {}): boolean {
  return scanCommand(command, context).isRisky;
}

export function riskAssessmentToEvidence(assessment: RiskAssessment): Evidence {
...
export function buildRiskEvent(input: BuildRiskEventInput): NotchEvent | undefined {
  const assessment = input.assessment ?? scanCommand(input.command, input);
  if (!assessment.isRisky) return undefined;

  const commandHash = input.commandHash ?? stableCommandHash(input.command);
  const event: NotchEvent = {
