...
function baseEvent(type, session, now, idKey, input) {
    return {
...
export function createExampleEvent(type, session, now) {
    switch (type) {
        case "confirm":
...
export function createMockEnvelopeSequence(options = {}) {
    ...
    if (scenario === "session")
        return envelopes;
    const eventTypes = scenario === "all" ? ["confirm", "risky-confirm", "risk", "result", "error"] : [scenario];
    for (const eventType of eventTypes) {
        envelopes.push(createEnvelope("notch.event.created", { event: createExampleEvent(eventType, session, now) }, source, now));
    }
    return envelopes;
}
...
export async function runMockCliSender(argv = process.argv.slice(2)) {
