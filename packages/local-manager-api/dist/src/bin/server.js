#!/usr/bin/env node
import { createLocalManagerApi } from "../index.js";
...
`;
function parseProcessSideEffectMode(value) {
    if (value === "mock" || value === "supervised")
        return value;
    throw new Error("Expected process side effect mode to be one of: mock, supervised.");
}
function parseOptionalIntervalMs(value, label) {
...
function parseArgs(argv) {
    ...
    let pendingActionTimeoutMs = parseOptionalIntervalMs(process.env.NOTCH_PENDING_ACTION_TIMEOUT_MS, "NOTCH_PENDING_ACTION_TIMEOUT_MS");
    let pendingActionSweepIntervalMs = parseOptionalIntervalMs(process.env.NOTCH_PENDING_ACTION_SWEEP_INTERVAL_MS, "NOTCH_PENDING_ACTION_SWEEP_INTERVAL_MS");
    let processSideEffectMode = parseProcessSideEffectMode(process.env.NOTCH_PROCESS_SIDE_EFFECT_MODE ?? "mock");
...
function createAppManagedLogger() {
    if (process.env.NOTCH_APP_MANAGED !== "1")
        return undefined;
...
const { host, port, processPersistenceFile, eventHistoryPersistenceFile, pendingActionTimeoutMs, pendingActionSweepIntervalMs, processSideEffectMode, } = parseArgs(process.argv.slice(2));
...
console.log(`Notch local manager API listening at ${address.url} (process side effects: ${processSideEffectMode}, pending action scheduler: ${pendingActionSweepIntervalMs && pendingActionSweepIntervalMs > 0
    ? `${pendingActionSweepIntervalMs}ms`
    : "off"}, pending action timeout: ${pendingActionTimeoutMs === undefined
    ? "default"
