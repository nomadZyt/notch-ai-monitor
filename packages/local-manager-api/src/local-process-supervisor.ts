...
import {
  stableCommandHash,
  type ProcessActionCompletionInput,
  type ProcessOwnershipInput,
  type ProcessSupervisor,
  type ProcessSupervisorRequest,
  type ProcessSupervisorResult,
  type RetryLaunchProfileInput,
  type RetrySupervisorRequest,
...
interface PendingTerminateAction {
  requestId: string;
  eventId: string;
  sessionId: string;
  runId: string;
...
function stableToken(prefix: string, value: string): string {
  return stableCommandHash(value).replace(/^cmd_/, `${prefix}_`);
}

function envForProfile(profile: RetrySupervisorRequest["launchProfile"]): NodeJS.ProcessEnv {
  if (!profile.envAllowlist) return process.env;
...
function sessionForRetry(
  request: RetrySupervisorRequest,
  sessionId: string,
  processId: number,
  now: string
): Session {
  ...
  if (request.session.project) session.project = request.session.project;
  return session;
}

function endStateForExit(exitCode: number | null, signal: NodeJS.Signals | null): {
  state: Extract<Session["state"], "completed" | "failed">;
  exitCode?: number;
  endReason?: string;
} {
...
export class LocalProcessSupervisor implements ProcessSupervisor {
  ...
  private readonly onSessionEnded: LocalProcessSupervisorOptions["onSessionEnded"];
  private readonly onProcessActionCompleted: LocalProcessSupervisorOptions["onProcessActionCompleted"];
  private readonly childrenBySessionId = new Map<string, OwnedChildProcess>();
  private readonly childrenByRunId = new Map<string, OwnedChildProcess>();
  private readonly adapterControlsBySessionId = new Map<string, AdapterControlRegistration>();
  private readonly pendingTerminateBySessionId = new Map<string, PendingTerminateAction>();
  ...
  startRetry(request: RetrySupervisorRequest): ProcessSupervisorResult {
    ...
    const supervisorTokenHash = stableToken(
      "supervisor",
      JSON.stringify({
        runId,
        sessionId,
    ...
    const retryLaunchProfile: RetryLaunchProfileInput = {
      sessionId,
    ...
    const processOwnership: ProcessOwnershipInput = {
      sessionId,
      runId,
      adapterId: request.launchProfile.adapterId,
      cwd: request.launchProfile.cwd,
  ...
  terminateGracefully(request: ProcessSupervisorRequest): ProcessSupervisorResult {
    ...
    if (!owned || owned.runId !== request.ownership.runId) {
      ...
      if (
        ...
        this.rememberPendingTerminate(
          request,
          `session:${request.session.id}#adapter-graceful-stop-completed`,
          `session:${request.session.id}#adapter-graceful-stop-failed`
        );
    ...
    this.rememberPendingTerminate(
      request,
      `session:${request.session.id}#graceful-stop-completed`,
      `session:${request.session.id}#graceful-stop-failed`
    );
    return {
      status: "accepted",
  ...
  close(): void {
    for (const owned of this.childrenByRunId.values()) {
      if (owned.exitCode !== undefined || owned.signal !== undefined) continue;
      owned.child.kill("SIGTERM");
    }
  }

  private requestAdapterGracefulStop(
    control: AdapterControlRegistration,
    ...
    const req = httpRequest(
      ...
      (res) => {
        ...
        res.on("data", (chunk: Buffer | string) => {
          ...
          if (totalBytes <= 64 * 1024) chunks.push(buffer);
        });
        res.on("end", () => {
          const bodyText = Buffer.concat(chunks).toString("utf8");
          const statusCode = res.statusCode ?? 0;
          if (statusCode >= 400 || statusCode === 0) {
            this.failPendingTerminate(
              request,
              "adapter 控制通道拒绝 graceful stop 请求",
              "adapter_control_rejected",
              `HTTP ${statusCode}${bodyText ? `: ${bodyText}` : ""}`
            );
            return;
          }
    ...
    req.on("error", (error) => {
      this.failPendingTerminate(
        request,
        "adapter 控制通道请求失败",
        "adapter_control_request_failed",
        error.message
      );
    });
    req.write(body);
  ...
  private adapterControlFailureFromPayload(value: unknown): { code: string; message: string } | null {
